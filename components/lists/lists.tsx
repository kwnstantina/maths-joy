...
const List=(props:Props):JSX.Element=> {
  ...
  const [selected, setSelected] = useState(isMultiple?[categories[0]]:categories[0]);
  useEffect(()=>{
      ...
      <Listbox value={selected} onChange={setSelected} multiple={isMultiple} refName={name}>
        <div className="relative mt-1">
          ...
          <Transition
            ...
            leaveTo="opacity-0"
          >
            <Listbox.Options className="absolute mt-1 max-h-60 w-44 overflow-auto rounded-md bg-white py-1 text-base shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none sm:text-sm">
